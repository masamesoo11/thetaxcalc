        ...
        <section className="py-8 border-t border-border/20">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <Link
              ...
              className="group flex items-start gap-3 rounded-lg border border-border/50 p-4 hover:border-emerald-500/30 hover:bg-emerald-500/5 transition-all"
            >
              <span className="text-lg">📈</span>
              <div>
                <span className="text-sm font-medium text-foreground group-hover:text-emerald-400 transition-colors">401(k) Retirement Calculator</span>
                <p className="text-xs text-muted-foreground mt-0.5">See how pre-tax contributions reduce your tax bill</p>
              </div>
            </Link>
            <Link
            ...
            >
              <span className="text-lg">🏠</span>
              <div>
                <span className="text-sm font-medium text-foreground group-hover:text-emerald-400 transition-colors">Mortgage Calculator</span>
                <p className="text-xs text-muted-foreground mt-0.5">Monthly payments and amortization schedule</p>
              </div>
            </Link>
            <Link
            ...
            >
              <span className="text-lg">📊</span>
              <div>
                <span className="text-sm font-medium text-foreground group-hover:text-emerald-400 transition-colors">Capital Gains Calculator</span>
                <p className="text-xs text-muted-foreground mt-0.5">Short-term vs long-term capital gains rates</p>
              </div>
            </Link>
            <Link
              href="/glossary"
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">FICA, standard deduction, and more explained</p>
              </div>
            </Link>
          </div>
        </section>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="rounded-xl border border-border/30 bg-card/50 p-5">
              ...
              <div className="flex flex-wrap gap-2">
                {(() => {
                  const idx = SALARY_AMOUNTS.indexOf(salary as (typeof SALARY_AMOUNTS)[number]);
                  const nearby: number[] = [];
                  // Pick 2-3 before and 2-3 after
                  for (let offset = 1; offset <= 6; offset++) {
                    if (idx - offset >= 0) nearby.push(SALARY_AMOUNTS[idx - offset]);
                    if (idx + offset < SALARY_AMOUNTS.length) nearby.push(SALARY_AMOUNTS[idx + offset]);
                    if (nearby.length >= 6) break;
                  }
                  // Sort and limit to 6
                  return nearby.sort((a, b) => a - b).slice(0, 6);
                })().map((amount) => (
                  <Link
                    key={amount}
                    href={`/salary/${amount}`}
                    className="inline-flex items-center gap-1.5 rounded-lg border border-border/50 bg-muted/20 px-3 py-2 text-sm font-medium text-foreground hover:border-emerald-500/30 hover:bg-emerald-500/10 hover:text-emerald-400 transition-all"
                  >
                    {formatSalary(amount)}
                  </Link>
                ))}
            ...
            <div className="rounded-xl border border-border/30 bg-card/50 p-5">
              ...
              <div className="space-y-2">
                <Link
                  href="/paycheck-calculator"
                  className="group flex items-center gap-2 rounded-lg bg-gradient-to-r from-emerald-600/15 to-teal-600/10 border border-emerald-500/20 px-4 py-2.5 text-sm font-semibold text-emerald-400 hover:from-emerald-600/25 hover:to-teal-600/15 transition-all"
                >
                  <BarChart3 className="h-4 w-4" />
                  Paycheck Calculator
                  <span className="ml-auto text-[10px] uppercase tracking-wider bg-emerald-500/20 rounded-full px-2 py-0.5">Primary</span>
                </Link>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    { href: '/relocation-calculator', label: 'Relocation' },
                    { href: '/compare', label: 'Compare States' },
                  ].map((item) => (
                    <Link
                      key={item.href}
                      href={item.href}
                      className="group flex items-center gap-1.5 rounded-lg border border-border/50 bg-muted/20 px-3 py-2 text-sm text-muted-foreground hover:border-emerald-500/30 hover:bg-emerald-500/5 hover:text-foreground transition-all"
                    >
            ...
            <div className="rounded-xl border border-border/30 bg-card/50 p-5">
              ...
              <div className="space-y-2">
                {[
                  { href: '/glossary', label: 'Tax Glossary', desc: 'Key terms explained simply' },
                  { href: '/federal-tax-brackets', label: 'Federal Tax Brackets', desc: '2026 bracket breakdown' },
                  { href: '/blog', label: 'Blog', desc: 'Tax tips & guides' },
                ].map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="group flex items-center gap-3 rounded-lg border border-border/50 bg-muted/20 px-4 py-3 hover:border-emerald-500/30 hover:bg-emerald-500/5 transition-all"
                  >
                    <div className="flex-1">
                      <span className="text-sm font-medium text-foreground group-hover:text-emerald-400 transition-colors">{item.label}</span>
                      <p className="text-xs text-muted-foreground">{item.desc}</p>
                    </div>
                    <ChevronRight className="h-4 w-4 text-muted-foreground/50 group-hover:text-emerald-400 transition-colors" />
                  </Link>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-xl border border-border/30 bg-card/50 p-5">
              ...
              <div className="space-y-2">
                {[
                  ...
                  const isCurrent = item.amount === salary;
                  return (
                    <Link
                      key={item.amount}
                      href={`/salary/${item.amount}`}
                      className={`group flex items-center justify-between rounded-lg border px-4 py-2.5 transition-all ${
                        isCurrent
                          ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-400'
                          : 'border-border/50 bg-muted/20 hover:border-emerald-500/30 hover:bg-emerald-500/5'
                      }`}
                    >
                      <div className="flex items-center gap-2">
                        <DollarSign className="h-3.5 w-3.5 text-emerald-500/70" />
                        <span className={`text-sm font-medium ${isCurrent ? 'text-emerald-400' : 'text-foreground'}`}>{formatSalary(item.amount)} After Tax</span>
                      </div>
                      <span className={`text-xs ${isCurrent ? 'text-emerald-400/70' : 'text-muted-foreground'}`}>{item.label}/yr</span>
                    </Link>
                  );
                })}
              </div>
