...
import { FAQSection, PENNSYLVANIA_FAQS } from './faq-sections';
import {
  calculatePaycheck,
  formatCurrency,
  formatPercent,
...
export function PennsylvaniaCalculator() {
  const stateKey = 'pennsylvania';
  const stateProfile = STATE_PROFILES.pennsylvania;
  ...
  const paBreakdown = useMemo(() => {
    const gross = result.grossAnnual;
    const pretax = retirement401k + hsaContribution;
    // PA has NO standard deduction and NO personal exemption
    const paTaxableIncome = Math.max(0, gross - pretax);
    const paTax = paTaxableIncome * stateProfile.incomeTaxRate;
    return { paTaxableIncome, paTax };
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          Pennsylvania flat 3.07% income tax with no standard deduction or personal exemption — complete take-home breakdown
        </p>
      </div>
        ...
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2 text-sm text-muted-foreground">
              <p className="font-medium text-foreground">How Pennsylvania Income Tax Works</p>
              <p>
                Pennsylvania uses a <strong className="text-emerald-400">flat tax rate of 3.07%</strong> on
                all taxable income — one of the lowest flat rates in the US. Unlike most states, Pennsylvania offers
                <strong className="text-emerald-400"> no standard deduction</strong> and
                <strong className="text-emerald-400"> no personal exemptions</strong> at the state level, meaning
                your gross income (minus pre-tax deductions like 401(k) and HSA) is fully subject to the 3.07% rate.
              </p>
              <p>
                Some municipalities also levy <strong className="text-emerald-400">local earned income taxes</strong> (typically 1–2%)
                which may apply depending on where you live. Philadelphia residents pay nearly 4%, while most other areas are 1–1.5%.
              </p>
              <p>
                For a $75,000 salary: Taxable = $75,000 (no state deductions). State tax = $75,000 × 3.07% = $2,302.50.
              </p>
            </div>
          </div>
        </CardContent>
        ...
        <div className="space-y-4 lg:col-span-3">
          <Card className="border-border/50 bg-card/80 backdrop-blur-sm">
            <CardHeader className="pb-4">
              ...
              <CardDescription>Enter your Pennsylvania salary for an instant calculation</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="pa-salary" className="text-sm font-medium">Salary / Hourly Rate</Label>
                  <div className="relative">
                    <DollarSign className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                    <Input id="pa-salary" type="number" min={0} value={salary || ''} onChange={(e) => setSalary(Number(e.target.value) || 0)} className="pl-9" placeholder="75000" />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="pa-frequency" className="text-sm font-medium">Pay Frequency</Label>
                  <Select value={payFrequency} onValueChange={(v) => setPayFrequency(v as PayFrequency)}>
                    <SelectTrigger id="pa-frequency"><SelectValue /></SelectTrigger>
                    <SelectContent>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {payFrequency === 'hourly' && (
                  <div className="space-y-2">
                    <Label htmlFor="pa-hours" className="text-sm font-medium">Hours / Week</Label>
                    <Input id="pa-hours" type="number" min={1} max={168} value={hoursPerWeek} onChange={(e) => setHoursPerWeek(Number(e.target.value) || 40)} />
                  </div>
                )}
                <div className="space-y-2">
                  <Label htmlFor="pa-filing" className="text-sm font-medium">Filing Status</Label>
                  <Select value={filingStatus} onValueChange={(v) => setFilingStatus(v as typeof filingStatus)}>
                    <SelectTrigger id="pa-filing"><SelectValue /></SelectTrigger>
                    <SelectContent>
            ...
            <Card className="border-border/50 bg-card/80 backdrop-blur-sm">
              ...
              <CollapsibleContent>
                <CardContent className="space-y-4 pt-0">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="space-y-2">
                      <Label htmlFor="pa-401k" className="text-sm font-medium">
                        <span className="flex items-center gap-1.5"><PiggyBank className="h-3.5 w-3.5 text-amber-400" />401(k) Annual</span>
                      </Label>
                      <div className="relative">
                        <DollarSign className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                        <Input id="pa-401k" type="number" min={0} max={24500} value={retirement401k || ''} onChange={(e) => setRetirement401k(Number(e.target.value) || 0)} className="pl-9" placeholder="0" />
                      </div>
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="pa-hsa" className="text-sm font-medium">
                        <span className="flex items-center gap-1.5"><Shield className="h-3.5 w-3.5 text-emerald-400" />HSA Annual</span>
                      </Label>
                      <div className="relative">
                        <DollarSign className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                        <Input id="pa-hsa" type="number" min={0} max={4150} value={hsaContribution || ''} onChange={(e) => setHsaContribution(Number(e.target.value) || 0)} className="pl-9" placeholder="0" />
                      </div>
                    </div>
                  </div>
                </CardContent>
        ...
        <div className="lg:col-span-2">
          <Card className="sticky top-20 border-emerald-500/20 bg-card/90 backdrop-blur-sm">
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-lg">
                <TrendingUp className="h-5 w-5 text-emerald-400" />Pennsylvania Take-Home
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2.5 rounded-lg bg-muted/30 p-3">
                ...
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1.5 text-sm text-muted-foreground"><TrendingDown className="h-3.5 w-3.5 text-amber-400" />PA State Tax (3.07%)</span>
              ...
              <div className="rounded-lg bg-amber-500/5 p-3">
                <p className="text-xs font-medium text-amber-400">Pennsylvania Tax Breakdown</p>
                <div className="mt-1.5 space-y-1 text-xs text-muted-foreground">
                  ...
                  <div className="flex justify-between"><span>Less: Standard Deduction</span><span>None</span></div>
                  <div className="flex justify-between"><span>Less: Personal Exemption</span><span>None</span></div>
                  <div className="flex justify-between"><span>PA Taxable Income</span><span>{formatCurrency(paBreakdown.paTaxableIncome)}</span></div>
                  <div className="flex justify-between"><span>PA Tax Rate</span><span>3.07%</span></div>
                  <div className="flex justify-between font-medium text-amber-400"><span>PA State Tax</span><span>{formatCurrency(paBreakdown.paTax)}</span></div>
                </div>
                <p className="mt-2 text-[10px] text-muted-foreground/70">
                  * Local earned income taxes (1–2% typically) may apply depending on your municipality
                </p>
              </div>
      ...
      <Card className="border-border/50 bg-card/80">
        <CardHeader>
          <CardTitle className="text-lg">2026 Pennsylvania Tax Rates &amp; Constants</CardTitle>
          <CardDescription>Key tax parameters for Pennsylvania paycheck calculations</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              <TableRow>
                ...
                <TableCell className="font-medium text-emerald-400">3.07% (Flat)</TableCell>
                <TableCell>One of the lowest flat rates in the US; applies to all taxable income</TableCell>
              </TableRow>
              <TableRow>
                <TableCell>Standard Deduction</TableCell>
                <TableCell className="font-medium">None</TableCell>
                <TableCell>Pennsylvania does not offer a standard deduction at the state level</TableCell>
              </TableRow>
              <TableRow>
                <TableCell>Personal Exemption</TableCell>
                <TableCell className="font-medium">None</TableCell>
                <TableCell>Pennsylvania does not offer personal exemptions at the state level</TableCell>
              </TableRow>
              <TableRow>
                <TableCell>Local Earned Income Tax</TableCell>
                <TableCell className="font-medium text-amber-400">1–2%</TableCell>
                <TableCell>Varies by municipality; Philadelphia ~4%, Pittsburgh ~3%, most others 1–1.5%</TableCell>
              </TableRow>
              <TableRow>
      ...
      <div className="rounded-xl border border-border/30 bg-muted/10 p-6" aria-hidden="true">
        <h2 className="text-xl font-bold text-foreground">Pennsylvania $75,000 Salary Example (2026)</h2>
        <div className="mt-3 space-y-1 text-sm text-muted-foreground">
          <p>Gross Annual Salary: $75,000.00</p>
          <p>PA Taxable Income: $75,000.00 (no state deductions)</p>
          <p>PA State Tax (3.07%): $2,302.50</p>
          <p>Federal Tax (after $16,100 std deduction): $7,670.00</p>
          <p>FICA Total (7.65%): $5,737.50</p>
          <p>Total Deductions: $15,710.00</p>
          <p>Net Annual Take-Home: $59,290.00</p>
          <p>Monthly Take-Home: $4,940.83</p>
        </div>
      </div>

      <FAQSection title="Pennsylvania Tax FAQ — Common Questions Answered" faqs={PENNSYLVANIA_FAQS} />
