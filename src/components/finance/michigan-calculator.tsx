...
import { FAQSection, MICHIGAN_FAQS } from './faq-sections';
import {
  calculatePaycheck,
  formatCurrency,
  formatPercent,
...
export function MichiganCalculator() {
  const stateKey = 'michigan';
  const stateProfile = STATE_PROFILES.michigan;
  ...
  const miBreakdown = useMemo(() => {
    ...
    const miTaxableIncome = Math.max(0, gross - pretax - personalExemption - standardDeduction);
    const miTax = miTaxableIncome * stateProfile.incomeTaxRate;
    return { miTaxableIncome, miTax, personalExemption, standardDeduction };
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          Michigan flat 4.25% income tax with personal exemption — complete take-home breakdown
        </p>
      </div>
        ...
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2 text-sm text-muted-foreground">
              <p className="font-medium text-foreground">How Michigan Income Tax Works</p>
              <p>
                Michigan uses a <strong className="text-emerald-400">flat tax rate of 4.25%</strong> on
                all taxable income. Michigan does not offer a standard deduction, but provides a <strong className="text-emerald-400">personal exemption</strong> ($5,500 single / $11,000 married / $5,500 head of household) which reduces your taxable income before the flat rate is applied.
              </p>
              <p>
                Some cities like <strong className="text-emerald-400">Detroit</strong> levy additional city income taxes (~2.4% for residents, ~1.2% for non-residents working in the city). These city taxes are not included in this calculator.
              </p>
              <p>
                For a $75,000 salary (single): Taxable income = $75,000 - $5,500 = $69,500. State tax = $69,500 × 4.25% = $2,953.75.
              </p>
            </div>
          </div>
        </CardContent>
        ...
        <div className="space-y-4 lg:col-span-3">
          <Card className="border-border/50 bg-card/80 backdrop-blur-sm">
            <CardHeader className="pb-4">
              ...
              <CardDescription>Enter your Michigan salary for an instant calculation</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="mi-salary" className="text-sm font-medium">Salary / Hourly Rate</Label>
                  <div className="relative">
                    <DollarSign className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                    <Input id="mi-salary" type="number" min={0} value={salary || ''} onChange={(e) => setSalary(Number(e.target.value) || 0)} className="pl-9" placeholder="75000" />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="mi-frequency" className="text-sm font-medium">Pay Frequency</Label>
                  <Select value={payFrequency} onValueChange={(v) => setPayFrequency(v as PayFrequency)}>
                    <SelectTrigger id="mi-frequency"><SelectValue /></SelectTrigger>
                    <SelectContent>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {payFrequency === 'hourly' && (
                  <div className="space-y-2">
                    <Label htmlFor="mi-hours" className="text-sm font-medium">Hours / Week</Label>
                    <Input id="mi-hours" type="number" min={1} max={168} value={hoursPerWeek} onChange={(e) => setHoursPerWeek(Number(e.target.value) || 40)} />
                  </div>
                )}
                <div className="space-y-2">
                  <Label htmlFor="mi-filing" className="text-sm font-medium">Filing Status</Label>
                  <Select value={filingStatus} onValueChange={(v) => setFilingStatus(v as typeof filingStatus)}>
                    <SelectTrigger id="mi-filing"><SelectValue /></SelectTrigger>
                    <SelectContent>
            ...
            <Card className="border-border/50 bg-card/80 backdrop-blur-sm">
              ...
              <CollapsibleContent>
                <CardContent className="space-y-4 pt-0">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="space-y-2">
                      <Label htmlFor="mi-401k" className="text-sm font-medium">
                        <span className="flex items-center gap-1.5"><PiggyBank className="h-3.5 w-3.5 text-amber-400" />401(k) Annual</span>
                      </Label>
                      <div className="relative">
                        <DollarSign className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                        <Input id="mi-401k" type="number" min={0} max={23500} value={retirement401k || ''} onChange={(e) => setRetirement401k(Number(e.target.value) || 0)} className="pl-9" placeholder="0" />
                      </div>
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="mi-hsa" className="text-sm font-medium">
                        <span className="flex items-center gap-1.5"><Shield className="h-3.5 w-3.5 text-emerald-400" />HSA Annual</span>
                      </Label>
                      <div className="relative">
                        <DollarSign className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                        <Input id="mi-hsa" type="number" min={0} max={4150} value={hsaContribution || ''} onChange={(e) => setHsaContribution(Number(e.target.value) || 0)} className="pl-9" placeholder="0" />
                      </div>
                    </div>
                  </div>
                </CardContent>
        ...
        <div className="lg:col-span-2">
          <Card className="sticky top-20 border-emerald-500/20 bg-card/90 backdrop-blur-sm">
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-lg">
                <TrendingUp className="h-5 w-5 text-emerald-400" />Michigan Take-Home
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2.5 rounded-lg bg-muted/30 p-3">
                ...
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1.5 text-sm text-muted-foreground"><TrendingDown className="h-3.5 w-3.5 text-amber-400" />MI State Tax (4.25%)</span>
              ...
              <div className="rounded-lg bg-amber-500/5 p-3">
                <p className="text-xs font-medium text-amber-400">Michigan Tax Breakdown</p>
                <div className="mt-1.5 space-y-1 text-xs text-muted-foreground">
                  <div className="flex justify-between"><span>Gross Income</span><span>{formatCurrency(result.grossAnnual)}</span></div>
                  <div className="flex justify-between"><span>Less: Personal Exemption</span><span>-{formatCurrency(miBreakdown.personalExemption)}</span></div>
                  <div className="flex justify-between"><span>MI Taxable Income</span><span>{formatCurrency(miBreakdown.miTaxableIncome)}</span></div>
                  <div className="flex justify-between"><span>MI Tax Rate</span><span>4.25%</span></div>
                  <div className="flex justify-between font-medium text-amber-400"><span>MI State Tax</span><span>{formatCurrency(miBreakdown.miTax)}</span></div>
                </div>
              </div>
      ...
      <Card className="border-border/50 bg-card/80">
        <CardHeader>
          <CardTitle className="text-lg">2026 Michigan Tax Rates &amp; Constants</CardTitle>
          <CardDescription>Key tax parameters for Michigan paycheck calculations</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              <TableRow>
                ...
                <TableCell className="font-medium text-emerald-400">4.25% (Flat)</TableCell>
                <TableCell>Flat rate applies to all taxable income after personal exemption</TableCell>
              </TableRow>
              <TableRow>
              ...
              <TableRow>
                ...
                <TableCell className="font-medium">$11,000</TableCell>
                <TableCell>$5,500 per person for married filing jointly</TableCell>
              </TableRow>
              <TableRow>
                <TableCell>Personal Exemption (Head of Household)</TableCell>
                <TableCell className="font-medium">$5,500</TableCell>
              ...
              <TableRow>
                ...
                <TableCell>Michigan does not offer a standard deduction</TableCell>
              </TableRow>
              <TableRow>
                <TableCell>Detroit City Tax</TableCell>
                <TableCell className="font-medium">~2.4%</TableCell>
                <TableCell>Applies to Detroit residents; ~1.2% for non-resident workers (not included in this calculator)</TableCell>
              </TableRow>
              <TableRow>
      ...
      <div className="rounded-xl border border-border/30 bg-muted/10 p-6" aria-hidden="true">
        <h2 className="text-xl font-bold text-foreground">Michigan $75,000 Salary Example (2026)</h2>
        <div className="mt-3 space-y-1 text-sm text-muted-foreground">
          <p>Gross Annual Salary: $75,000.00</p>
          <p>Michigan Personal Exemption: -$5,500.00</p>
          <p>Michigan Taxable Income: $69,500.00</p>
          <p>Michigan State Tax (4.25%): $2,953.75</p>
          <p>Federal Tax (after $16,100 std deduction): $7,670.00</p>
          <p>FICA Total (7.65%): $5,737.50</p>
          <p>Total Deductions: $16,361.25</p>
          <p>Net Annual Take-Home: $58,638.75</p>
          <p>Monthly Take-Home: $4,886.56</p>
        </div>
      </div>

      <FAQSection title="Michigan Tax FAQ — Common Questions Answered" faqs={MICHIGAN_FAQS} />
