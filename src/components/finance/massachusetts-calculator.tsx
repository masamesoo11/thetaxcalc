...
import { FAQSection, MASSACHUSETTS_FAQS } from './faq-sections';
import {
  calculatePaycheck,
  formatCurrency,
  formatPercent,
...
export function MassachusettsCalculator() {
  const stateKey = 'massachusetts';
  const stateProfile = STATE_PROFILES.massachusetts;
  ...
  const maBreakdown = useMemo(() => {
    const gross = result.grossAnnual;
    const pretax = retirement401k + hsaContribution;
    const maTaxableIncome = Math.max(0, gross - pretax);
    let maTax: number;
    let maRate: string;
    if (maTaxableIncome > 1000000) {
      maTax = 1000000 * 0.05 + (maTaxableIncome - 1000000) * 0.09;
      maRate = '5% + 4% surtax over $1M';
    } else {
      maTax = maTaxableIncome * 0.05;
      maRate = '5%';
    }
    return { maTaxableIncome, maTax, maRate };
  }, [result.grossAnnual, retirement401k, hsaContribution]);
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          Massachusetts 5% flat income tax with 4% millionaire&apos;s surtax — complete take-home breakdown
        </p>
      </div>
        ...
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2 text-sm text-muted-foreground">
              <p className="font-medium text-foreground">How Massachusetts Income Tax Works</p>
              <p>
                Massachusetts uses a <strong className="text-emerald-400">flat 5% income tax rate</strong> on most income.
                Starting in 2023, a <strong className="text-amber-400">4% surtax (&quot;millionaire&apos;s tax&quot;)</strong> applies
                to income exceeding $1,000,000, bringing the effective top rate to 9%.
                Massachusetts does not offer a standard deduction — the flat rate applies to your full taxable income
                after pre-tax deductions (401(k), HSA).
              </p>
              <p>
                For a $75,000 salary (single): MA Taxable Income = $75,000. State tax = $75,000 × 5% = $3,750.
              </p>
            </div>
          </div>
        </CardContent>
        ...
        <div className="space-y-4 lg:col-span-3">
          <Card className="border-border/50 bg-card/80 backdrop-blur-sm">
            <CardHeader className="pb-4">
              ...
              <CardDescription>Enter your Massachusetts salary for an instant calculation</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="ma-salary" className="text-sm font-medium">Salary / Hourly Rate</Label>
                  <div className="relative">
                    <DollarSign className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                    <Input id="ma-salary" type="number" min={0} value={salary || ''} onChange={(e) => setSalary(Number(e.target.value) || 0)} className="pl-9" placeholder="75000" />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="ma-frequency" className="text-sm font-medium">Pay Frequency</Label>
                  <Select value={payFrequency} onValueChange={(v) => setPayFrequency(v as PayFrequency)}>
                    <SelectTrigger id="ma-frequency"><SelectValue /></SelectTrigger>
                    <SelectContent>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {payFrequency === 'hourly' && (
                  <div className="space-y-2">
                    <Label htmlFor="ma-hours" className="text-sm font-medium">Hours / Week</Label>
                    <Input id="ma-hours" type="number" min={1} max={168} value={hoursPerWeek} onChange={(e) => setHoursPerWeek(Number(e.target.value) || 40)} />
                  </div>
                )}
                <div className="space-y-2">
                  <Label htmlFor="ma-filing" className="text-sm font-medium">Filing Status</Label>
                  <Select value={filingStatus} onValueChange={(v) => setFilingStatus(v as typeof filingStatus)}>
                    <SelectTrigger id="ma-filing"><SelectValue /></SelectTrigger>
                    <SelectContent>
            ...
            <Card className="border-border/50 bg-card/80 backdrop-blur-sm">
              ...
              <CollapsibleContent>
                <CardContent className="space-y-4 pt-0">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="space-y-2">
                      <Label htmlFor="ma-401k" className="text-sm font-medium">
                        <span className="flex items-center gap-1.5"><PiggyBank className="h-3.5 w-3.5 text-amber-400" />401(k) Annual</span>
                      </Label>
                      <div className="relative">
                        <DollarSign className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                        <Input id="ma-401k" type="number" min={0} max={24500} value={retirement401k || ''} onChange={(e) => setRetirement401k(Number(e.target.value) || 0)} className="pl-9" placeholder="0" />
                      </div>
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="ma-hsa" className="text-sm font-medium">
                        <span className="flex items-center gap-1.5"><Shield className="h-3.5 w-3.5 text-emerald-400" />HSA Annual</span>
                      </Label>
                      <div className="relative">
                        <DollarSign className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                        <Input id="ma-hsa" type="number" min={0} max={4150} value={hsaContribution || ''} onChange={(e) => setHsaContribution(Number(e.target.value) || 0)} className="pl-9" placeholder="0" />
                      </div>
                    </div>
                  </div>
                </CardContent>
        ...
        <div className="lg:col-span-2">
          <Card className="sticky top-20 border-emerald-500/20 bg-card/90 backdrop-blur-sm">
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-lg">
                <TrendingUp className="h-5 w-5 text-emerald-400" />Massachusetts Take-Home
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2.5 rounded-lg bg-muted/30 p-3">
                ...
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1.5 text-sm text-muted-foreground"><TrendingDown className="h-3.5 w-3.5 text-amber-400" />MA State Tax</span>
              ...
              <div className="rounded-lg bg-amber-500/5 p-3">
                <p className="text-xs font-medium text-amber-400">Massachusetts Tax Breakdown</p>
                <div className="mt-1.5 space-y-1 text-xs text-muted-foreground">
                  ...
                  <div className="flex justify-between"><span>MA Taxable Income</span><span>{formatCurrency(maBreakdown.maTaxableIncome)}</span></div>
                  <div className="flex justify-between"><span>MA Tax Rate</span><span>{maBreakdown.maRate}</span></div>
                  <div className="flex justify-between font-medium text-amber-400"><span>MA State Tax</span><span>{formatCurrency(maBreakdown.maTax)}</span></div>
                </div>
              </div>
      ...
      <Card className="border-border/50 bg-card/80">
        <CardHeader>
          <CardTitle className="text-lg">2026 Massachusetts Tax Rates &amp; Constants</CardTitle>
          <CardDescription>Key tax parameters for Massachusetts paycheck calculations</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              <TableRow>
                ...
                <TableCell className="font-medium text-emerald-400">5% (Flat)</TableCell>
                <TableCell>Applies to all taxable income</TableCell>
              </TableRow>
              <TableRow>
                <TableCell>Millionaire&apos;s Surtax</TableCell>
                <TableCell className="font-medium text-amber-400">+4% over $1M</TableCell>
                <TableCell>4% surtax on income above $1,000,000 (effective rate: 9%)</TableCell>
              </TableRow>
              <TableRow>
                <TableCell>MA Standard Deduction</TableCell>
                <TableCell className="font-medium">None</TableCell>
                <TableCell>Massachusetts does not offer a standard deduction</TableCell>
              </TableRow>
              <TableRow>
              ...
              <TableRow>
                <TableCell>MA Sales Tax</TableCell>
                <TableCell className="font-medium">6.25%</TableCell>
                <TableCell>State sales tax rate; no local sales tax added</TableCell>
              </TableRow>
              <TableRow>
      ...
      <div className="rounded-xl border border-border/30 bg-muted/10 p-6" aria-hidden="true">
        <h2 className="text-xl font-bold text-foreground">Massachusetts $75,000 Salary Example (2026)</h2>
        <div className="mt-3 space-y-1 text-sm text-muted-foreground">
          <p>Gross Annual Salary: $75,000.00</p>
          <p>Massachusetts State Tax (5%): $3,750.00</p>
          <p>Federal Tax (after $16,100 std deduction): $7,670.00</p>
          <p>FICA Total (7.65%): $5,737.50</p>
          <p>Total Deductions: $17,157.50</p>
          <p>Net Annual Take-Home: $57,842.50</p>
          <p>Monthly Take-Home: $4,820.21</p>
        </div>
      </div>

      <FAQSection title="Massachusetts Tax FAQ — Common Questions Answered" faqs={MASSACHUSETTS_FAQS} />
