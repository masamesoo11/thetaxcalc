...
import { FAQSection, TENNESSEE_FAQS } from './faq-sections';
import {
  calculatePaycheck,
  formatCurrency,
  formatPercent,
...
export function TennesseeCalculator() {
  const stateKey = 'tennessee';
  ...
  const [homeValue, setHomeValue] = useState<number>(() => hashParams.homeValue ? Number(hashParams.homeValue) : 285000);
  const [annualSpending, setAnnualSpending] = useState<number>(() => hashParams.spending ? Number(hashParams.spending) : 46000);

  useEffect(() => {
    updateHashState('tennessee', {
      salary,
  ...
  const colResult = useMemo(() => {
    const propertyTaxRate = 0.0064;
    const salesTaxRate = 0.0955;
    const annualPropertyTax = homeValue * propertyTaxRate;
    const estimatedSalesTax = annualSpending * salesTaxRate * 0.4;
    const totalAnnualBurden = annualPropertyTax + estimatedSalesTax;
    return { annualPropertyTax, estimatedSalesTax, totalAnnualBurden, totalMonthlyBurden: totalAnnualBurden / 12 };
  }, [homeValue, annualSpending]);
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          Zero state income tax — but factor in sales &amp; property tax for the full picture
        </p>
      </div>
        ...
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2 text-sm text-muted-foreground">
              <p className="font-medium text-foreground">Tennessee: No State Income Tax on Wages</p>
              <p>
                Tennessee is one of nine U.S. states with <strong className="text-emerald-400">0% state income tax</strong> on wages.
                The Hall tax on dividends and interest was fully eliminated as of January 1, 2021.
                Your paycheck is only subject to federal tax and FICA. However, Tennessee has one of the
                <strong className="text-amber-400"> highest combined sales tax rates</strong> in the nation
                (7% state + up to 2.75% local, averaging ~9.55%). Property taxes are relatively low at ~0.64%.
              </p>
            </div>
          </div>
        </CardContent>
        ...
        <div className="space-y-4 lg:col-span-3">
          <Card className="border-border/50 bg-card/80 backdrop-blur-sm">
            <CardHeader className="pb-4">
              ...
              <CardDescription>Enter your Tennessee salary — no state tax deducted!</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="tn-salary" className="text-sm font-medium">Salary / Hourly Rate</Label>
                  <div className="relative">
                    <DollarSign className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                    <Input id="tn-salary" type="number" min={0} value={salary || ''} onChange={(e) => setSalary(Number(e.target.value) || 0)} className="pl-9" placeholder="75000" />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="tn-frequency" className="text-sm font-medium">Pay Frequency</Label>
                  <Select value={payFrequency} onValueChange={(v) => setPayFrequency(v as PayFrequency)}>
                    <SelectTrigger id="tn-frequency"><SelectValue /></SelectTrigger>
                    <SelectContent>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {payFrequency === 'hourly' && (
                  <div className="space-y-2">
                    <Label htmlFor="tn-hours" className="text-sm font-medium">Hours / Week</Label>
                    <Input id="tn-hours" type="number" min={1} max={168} value={hoursPerWeek} onChange={(e) => setHoursPerWeek(Number(e.target.value) || 40)} />
                  </div>
                )}
                <div className="space-y-2">
                  <Label htmlFor="tn-filing" className="text-sm font-medium">Filing Status</Label>
                  <Select value={filingStatus} onValueChange={(v) => setFilingStatus(v as typeof filingStatus)}>
                    <SelectTrigger id="tn-filing"><SelectValue /></SelectTrigger>
                    <SelectContent>
          ...
          <Card className="border-amber-500/20 bg-card/80 backdrop-blur-sm">
            <CardHeader className="pb-4">
              ...
              <CardDescription>Tennessee has no income tax, but sales &amp; property taxes create a real burden</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="tn-home" className="text-sm font-medium">
                    <span className="flex items-center gap-1.5"><Home className="h-3.5 w-3.5 text-amber-400" />Home Value</span>
                  </Label>
                  <div className="relative">
                    <DollarSign className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                    <Input id="tn-home" type="number" min={0} value={homeValue || ''} onChange={(e) => setHomeValue(Number(e.target.value) || 0)} className="pl-9" placeholder="285000" />
                  </div>
                  <p className="text-xs text-muted-foreground">TN avg effective property tax rate: 0.64%</p>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="tn-spending" className="text-sm font-medium">
                    <span className="flex items-center gap-1.5"><ShoppingCart className="h-3.5 w-3.5 text-amber-400" />Annual Consumer Spending</span>
                  </Label>
                  <div className="relative">
                    <DollarSign className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                    <Input id="tn-spending" type="number" min={0} value={annualSpending || ''} onChange={(e) => setAnnualSpending(Number(e.target.value) || 0)} className="pl-9" placeholder="46000" />
                  </div>
                  <p className="text-xs text-muted-foreground">TN avg combined sales tax rate: 9.55%</p>
                </div>
              </div>
              ...
              <div className="rounded-lg bg-amber-500/5 p-4 space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-medium text-amber-400">{formatCurrency(colResult.estimatedSalesTax)}</span>
                </div>
        ...
        <div className="lg:col-span-2">
          <Card className="sticky top-20 border-emerald-500/20 bg-card/90 backdrop-blur-sm">
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-lg">
                <TrendingUp className="h-5 w-5 text-emerald-400" />Tennessee Take-Home
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="rounded-lg bg-emerald-500/5 p-3 text-center">
                ...
                <p className="text-xs text-emerald-400/60">Tennessee has 0% income tax</p>
              </div>
      ...
      <Card className="border-border/50 bg-card/80">
        <CardHeader>
          <CardTitle className="text-lg">2026 Tennessee Tax Rates &amp; Constants</CardTitle>
          <CardDescription>Key tax parameters for Tennessee paycheck calculations</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              <TableRow>
                <TableCell>Tennessee State Income Tax</TableCell>
                <TableCell className="font-medium text-emerald-400">0%</TableCell>
                <TableCell>Hall tax fully eliminated as of Jan 1, 2021</TableCell>
              </TableRow>
              <TableRow>
                <TableCell>TN State Sales Tax</TableCell>
                <TableCell className="font-medium">7%</TableCell>
                <TableCell>One of the highest state sales tax rates in the nation</TableCell>
              </TableRow>
              <TableRow>
                <TableCell>TN Avg Combined Sales Tax</TableCell>
                <TableCell className="font-medium">~9.55%</TableCell>
                <TableCell>7% state + up to 2.75% local option taxes</TableCell>
              </TableRow>
              <TableRow>
                <TableCell>TN Avg Property Tax Rate</TableCell>
                <TableCell className="font-medium">~0.64%</TableCell>
                <TableCell>Below national average of ~1.1%</TableCell>
              </TableRow>
              <TableRow>
              ...
              <TableRow>
                <TableCell>Social Security Rate</TableCell>
                <TableCell className="font-medium">6.2%</TableCell>
                <TableCell>Up to {formatCurrency(FICA_2026.socialSecurityWageCap)} wage cap</TableCell>
              </TableRow>
              <TableRow>
      ...
      <div className="rounded-xl border border-border/30 bg-muted/10 p-6" aria-hidden="true">
        <h2 className="text-xl font-bold text-foreground">Tennessee $75,000 Salary Example (2026)</h2>
        <div className="mt-3 space-y-1 text-sm text-muted-foreground">
          <p>Gross Annual Salary: $75,000.00</p>
          <p>Tennessee State Tax: $0.00 (0% — no state income tax)</p>
      ...
      <FAQSection title="Tennessee Tax FAQ — No Income Tax, But What About Sales & Property Tax?" faqs={TENNESSEE_FAQS} />
