...
import { FAQSection, OREGON_FAQS } from './faq-sections';
...
export function OregonCalculator() {
  const stateKey = 'oregon';
  ...
  useEffect(() => {
    updateHashState('oregon', { salary, frequency: payFrequency, hours: hoursPerWeek, filing: filingStatus, k401k: retirement401k, hsa: hsaContribution });
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          Oregon progressive income tax (4.75%–9.9%) with no sales tax — complete take-home breakdown
        </p>
      </div>
        ...
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2 text-sm text-muted-foreground">
              <p className="font-medium text-foreground">How Oregon Income Tax Works</p>
              <p>
                Oregon uses a <strong className="text-emerald-400">progressive tax system with brackets from 4.75% to 9.9%</strong>,
                among the highest top rates in the nation. Oregon offers a <strong className="text-emerald-400">standard deduction</strong>
                ($2,605 single / $5,210 married). Notably, Oregon has <strong className="text-emerald-400">no state sales tax</strong>,
                which partially offsets the high income tax. The state also provides a &quot;kicker&quot; refund when revenues exceed projections.
              </p>
              <p>
                For a $75,000 salary (single): OR standard deduction = $2,605. OR taxable income = $75,000 - $2,605 = $72,395. State tax is calculated using progressive brackets.
              </p>
            </div>
          </div>
        </CardContent>
        ...
        <div className="space-y-4 lg:col-span-3">
          <Card className="border-border/50 bg-card/80 backdrop-blur-sm">
            <CardHeader className="pb-4">
              ...
              <CardDescription>Enter your Oregon salary for an instant calculation</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="or-salary" className="text-sm font-medium">Salary / Hourly Rate</Label>
                  <div className="relative">
                    <DollarSign className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                    <Input id="or-salary" type="number" min={0} value={salary || ''} onChange={(e) => setSalary(Number(e.target.value) || 0)} className="pl-9" placeholder="75000" />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="or-frequency" className="text-sm font-medium">Pay Frequency</Label>
                  <Select value={payFrequency} onValueChange={(v) => setPayFrequency(v as PayFrequency)}>
                    <SelectTrigger id="or-frequency"><SelectValue /></SelectTrigger>
                    <SelectContent>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {payFrequency === 'hourly' && (
                  <div className="space-y-2">
                    <Label htmlFor="or-hours" className="text-sm font-medium">Hours / Week</Label>
                    <Input id="or-hours" type="number" min={1} max={168} value={hoursPerWeek} onChange={(e) => setHoursPerWeek(Number(e.target.value) || 40)} />
                  </div>
                )}
                <div className="space-y-2">
                  <Label htmlFor="or-filing" className="text-sm font-medium">Filing Status</Label>
                  <Select value={filingStatus} onValueChange={(v) => setFilingStatus(v as typeof filingStatus)}>
                    <SelectTrigger id="or-filing"><SelectValue /></SelectTrigger>
                    <SelectContent>
            ...
            <Card className="border-border/50 bg-card/80 backdrop-blur-sm">
              ...
              <CollapsibleContent>
                <CardContent className="space-y-4 pt-0">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="space-y-2">
                      <Label htmlFor="or-401k" className="text-sm font-medium">
                        <span className="flex items-center gap-1.5"><PiggyBank className="h-3.5 w-3.5 text-amber-400" />401(k) Annual</span>
                      </Label>
                      <div className="relative">
                        <DollarSign className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                        <Input id="or-401k" type="number" min={0} max={24500} value={retirement401k || ''} onChange={(e) => setRetirement401k(Number(e.target.value) || 0)} className="pl-9" placeholder="0" />
                      </div>
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="or-hsa" className="text-sm font-medium">
                        <span className="flex items-center gap-1.5"><Shield className="h-3.5 w-3.5 text-emerald-400" />HSA Annual</span>
                      </Label>
                      <div className="relative">
                        <DollarSign className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                        <Input id="or-hsa" type="number" min={0} max={4150} value={hsaContribution || ''} onChange={(e) => setHsaContribution(Number(e.target.value) || 0)} className="pl-9" placeholder="0" />
                      </div>
                    </div>
                  </div>
                </CardContent>
        ...
        <div className="lg:col-span-2">
          <Card className="sticky top-20 border-emerald-500/20 bg-card/90 backdrop-blur-sm">
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-lg">
                <TrendingUp className="h-5 w-5 text-emerald-400" />Oregon Take-Home
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2.5 rounded-lg bg-muted/30 p-3">
                ...
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1.5 text-sm text-muted-foreground"><TrendingDown className="h-3.5 w-3.5 text-amber-400" />OR State Tax</span>
      ...
      <Card className="border-border/50 bg-card/80">
        <CardHeader>
          <CardTitle className="text-lg">2026 Oregon Tax Rates &amp; Constants</CardTitle>
          <CardDescription>Key tax parameters for Oregon paycheck calculations</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              <TableRow><TableCell>State Income Tax</TableCell><TableCell className="font-medium text-emerald-400">4.75%–9.9% (Progressive)</TableCell><TableCell>4 brackets; top rate above $125,000</TableCell></TableRow>
              <TableRow><TableCell>Standard Deduction (Single)</TableCell><TableCell className="font-medium">$2,605</TableCell><TableCell>Subtracted from gross before tax computation</TableCell></TableRow>
              <TableRow><TableCell>Standard Deduction (Married)</TableCell><TableCell className="font-medium">$5,210</TableCell><TableCell>Married filing jointly</TableCell></TableRow>
              <TableRow><TableCell>Oregon Sales Tax</TableCell><TableCell className="font-medium text-emerald-400">0%</TableCell><TableCell>Oregon is one of 5 states with no sales tax</TableCell></TableRow>
      ...
      <div className="rounded-xl border border-border/30 bg-muted/10 p-6" aria-hidden="true">
        <h2 className="text-xl font-bold text-foreground">Oregon $75,000 Salary Example (2026)</h2>
        <div className="mt-3 space-y-1 text-sm text-muted-foreground">
          <p>Gross Annual Salary: $75,000.00</p>
          <p>OR Standard Deduction: -$2,605.00</p>
          <p>OR Taxable Income: $72,395.00</p>
          <p>OR State Tax (4.75%–9.9%): ~$5,280</p>
          <p>Federal Tax (after $16,100 std deduction): $7,670.00</p>
          <p>FICA Total (7.65%): $5,737.50</p>
          <p>Total Deductions: ~$18,687.50</p>
          <p>Net Annual Take-Home: ~$56,312.50</p>
          <p>Monthly Take-Home: ~$4,692.71</p>
        </div>
      </div>

      <FAQSection title="Oregon Tax FAQ — Common Questions Answered" faqs={OREGON_FAQS} />
