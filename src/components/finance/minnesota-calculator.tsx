...
import { FAQSection, MINNESOTA_FAQS } from './faq-sections';
...
export function MinnesotaCalculator() {
  const stateKey = 'minnesota';
  ...
  useEffect(() => {
    updateHashState('minnesota', { salary, frequency: payFrequency, hours: hoursPerWeek, filing: filingStatus, k401k: retirement401k, hsa: hsaContribution });
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          Minnesota progressive income tax (5.35%–9.85%) with standard deduction — complete take-home breakdown
        </p>
      </div>
        ...
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2 text-sm text-muted-foreground">
              <p className="font-medium text-foreground">How Minnesota Income Tax Works</p>
              <p>
                Minnesota uses a <strong className="text-sky-400">progressive tax system with brackets from 5.35% to 9.85%</strong>,
                making it one of the highest top rates in the nation. Minnesota offers a <strong className="text-sky-400">standard deduction</strong>
                ($14,575 single / $29,150 married / $21,850 head of household). The state also provides a working family credit
                and various other credits for lower-income residents.
              </p>
              <p>
                For a $75,000 salary (single): MN standard deduction = $14,575. MN taxable income = $75,000 - $14,575 = $60,425. State tax is calculated using progressive brackets.
              </p>
            </div>
          </div>
        </CardContent>
        ...
        <div className="space-y-4 lg:col-span-3">
          <Card className="border-border/50 bg-card/80 backdrop-blur-sm">
            <CardHeader className="pb-4">
              ...
              <CardDescription>Enter your Minnesota salary for an instant calculation</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="mn-salary" className="text-sm font-medium">Salary / Hourly Rate</Label>
                  <div className="relative">
                    <DollarSign className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                    <Input id="mn-salary" type="number" min={0} value={salary || ''} onChange={(e) => setSalary(Number(e.target.value) || 0)} className="pl-9" placeholder="75000" />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="mn-frequency" className="text-sm font-medium">Pay Frequency</Label>
                  <Select value={payFrequency} onValueChange={(v) => setPayFrequency(v as PayFrequency)}>
                    <SelectTrigger id="mn-frequency"><SelectValue /></SelectTrigger>
                    <SelectContent>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {payFrequency === 'hourly' && (
                  <div className="space-y-2">
                    <Label htmlFor="mn-hours" className="text-sm font-medium">Hours / Week</Label>
                    <Input id="mn-hours" type="number" min={1} max={168} value={hoursPerWeek} onChange={(e) => setHoursPerWeek(Number(e.target.value) || 40)} />
                  </div>
                )}
                <div className="space-y-2">
                  <Label htmlFor="mn-filing" className="text-sm font-medium">Filing Status</Label>
                  <Select value={filingStatus} onValueChange={(v) => setFilingStatus(v as typeof filingStatus)}>
                    <SelectTrigger id="mn-filing"><SelectValue /></SelectTrigger>
                    <SelectContent>
            ...
            <Card className="border-border/50 bg-card/80 backdrop-blur-sm">
              ...
              <CollapsibleContent>
                <CardContent className="space-y-4 pt-0">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="space-y-2">
                      <Label htmlFor="mn-401k" className="text-sm font-medium">
                        <span className="flex items-center gap-1.5"><PiggyBank className="h-3.5 w-3.5 text-amber-400" />401(k) Annual</span>
                      </Label>
                      <div className="relative">
                        <DollarSign className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                        <Input id="mn-401k" type="number" min={0} max={23500} value={retirement401k || ''} onChange={(e) => setRetirement401k(Number(e.target.value) || 0)} className="pl-9" placeholder="0" />
                      </div>
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="mn-hsa" className="text-sm font-medium">
                        <span className="flex items-center gap-1.5"><Shield className="h-3.5 w-3.5 text-sky-400" />HSA Annual</span>
                      </Label>
                      <div className="relative">
                        <DollarSign className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                        <Input id="mn-hsa" type="number" min={0} max={4150} value={hsaContribution || ''} onChange={(e) => setHsaContribution(Number(e.target.value) || 0)} className="pl-9" placeholder="0" />
                      </div>
                    </div>
                  </div>
                </CardContent>
        ...
        <div className="lg:col-span-2">
          <Card className="sticky top-20 border-sky-500/20 bg-card/90 backdrop-blur-sm">
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-lg">
                <TrendingUp className="h-5 w-5 text-sky-400" />Minnesota Take-Home
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2.5 rounded-lg bg-muted/30 p-3">
                ...
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1.5 text-sm text-muted-foreground"><TrendingDown className="h-3.5 w-3.5 text-amber-400" />MN State Tax</span>
      ...
      <Card className="border-border/50 bg-card/80">
        <CardHeader>
          <CardTitle className="text-lg">2026 Minnesota Tax Rates &amp; Constants</CardTitle>
          <CardDescription>Key tax parameters for Minnesota paycheck calculations</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              <TableRow><TableCell>State Income Tax</TableCell><TableCell className="font-medium text-sky-400">5.35%–9.85% (Progressive)</TableCell><TableCell>4 brackets; top rate above $210,260</TableCell></TableRow>
              <TableRow><TableCell>Standard Deduction (Single)</TableCell><TableCell className="font-medium">$14,575</TableCell><TableCell>Subtracted from gross before tax computation</TableCell></TableRow>
              <TableRow><TableCell>Standard Deduction (Married)</TableCell><TableCell className="font-medium">$29,150</TableCell><TableCell>Married filing jointly</TableCell></TableRow>
              <TableRow><TableCell>Standard Deduction (HOH)</TableCell><TableCell className="font-medium">$21,850</TableCell><TableCell>Head of household</TableCell></TableRow>
      ...
      <div className="rounded-xl border border-border/30 bg-muted/10 p-6" aria-hidden="true">
        <h2 className="text-xl font-bold text-foreground">Minnesota $75,000 Salary Example (2026)</h2>
        <div className="mt-3 space-y-1 text-sm text-muted-foreground">
          <p>Gross Annual Salary: $75,000.00</p>
          <p>MN Standard Deduction: -$14,575.00</p>
          <p>MN Taxable Income: $60,425.00</p>
          <p>MN State Tax (5.35%–9.85%): ~$3,475</p>
          <p>Federal Tax (after $16,100 std deduction): $7,670.00</p>
          <p>FICA Total (7.65%): $5,737.50</p>
          <p>Total Deductions: ~$16,882.50</p>
          <p>Net Annual Take-Home: ~$58,117.50</p>
          <p>Monthly Take-Home: ~$4,843.13</p>
        </div>
      </div>

      <FAQSection title="Minnesota Tax FAQ — Common Questions Answered" faqs={MINNESOTA_FAQS} />
